...
def start_server():
    ...
    s_home = bottle.template("home.tpl", site)
    return s_home
...
def get_info_authors():
    ...
    if result["PUBLICATIONS"] > 0:
        site_author_infos = bottle.template("author_infos.tpl", site, isauthor=True)
    else:
        site_author_infos = bottle.template("author_infos.tpl", site, isauthor=False)
...
def get_publications_author():
    ...
    if result:
        site_elements = {"title": title_server,
                         ...
                         "author": name,
                         "contents": result}
        s = bottle.template("author_publications.tpl", site_elements, list_content=True)
    else:
        site_elements = {"title": "publications scientifiques",
                         "h1": "Publications d' article d' un auteur",
                         "author": name,
                         "contents": []}
        s = bottle.template("author_publications.tpl", site_elements, list_content=False)
    return s
...
def get_coauthors_author():
    ...
    if result:
        site_elements = {"title": title_server,
                         ...
                         "author": name,
                         "contents": result}
        site_author_coauthors = bottle.template("author_publications.tpl", site_elements, list_content=True)
    else:
        site_elements = {"title": "publications scientifiques",
                         "h1": "Liste de co-auteurs d'un auteur",
                         "author": name,
                         "contents": []}
        site_author_coauthors = bottle.template("author_coauthors.tpl", site_elements, list_content=False)
...
@bottle.post("/authors/search") #recuperer les donnees avec post
def get_search_author():
    ...
    if result:
        site_elements = {"title": title_server,
                         ...
                         "name": name,
                         "contents": result}
        site_author_publications = bottle.template("author_search.tpl", site_elements, list_content=True)
    else:
        site_elements = {"title": "Liste d'auteurs",
                         "h1": "Liste d'auteurs",
                         "name": name,
                         "contents": []}
        site_author_publications = bottle.template("author_search.tpl", site_elements, list_content=False)
...
def get_authors_distance():
    ...
    site = {"title": title_server,
            ...
            "distance": result}
    if result == 0:
        site_author_distance = bottle.template("author_distance.tpl", site, connect=False)
    else:
        site_author_distance = bottle.template("author_distance.tpl", site, connect=True)
